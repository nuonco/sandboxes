locals {
  k8s_exec = [{
    ...
    args = ["token", "-i", module.sandbox.cluster.cluster_name, "-r", var.assume_role_arn]
  }]
}
...
provider "kubernetes" {
  host                   = module.sandbox.cluster.cluster_endpoint
  cluster_ca_certificate = base64decode(module.sandbox.cluster.cluster_certificate_authority_data)
